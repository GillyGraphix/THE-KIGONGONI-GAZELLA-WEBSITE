...
@php
    ...
    $guests   = request('guests', '2 Adults');
...
<div class="bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 transition-colors duration-300">
    <div class="container mx-auto px-4 py-3">
        <nav class="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500">
            ...
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-kigongoniOrange">{{ __('Standard Double Room') }}</span>
        </nav>
    </div>
</div>
...
<div class="bg-white dark:bg-gray-900 transition-colors duration-300 min-h-screen">
    <div class="container mx-auto px-4 py-12 max-w-7xl">
            ...
            <div class="flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-black text-kigongoniBlue dark:text-white">{{ $guests }}</p>
                    </div>
                </div>
        ...
        {{-- ── INLINE CHANGE-DATES CALENDAR (hidden by default) ───── --}}
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-kigongoniOrange/30 dark:border-kigongoniOrange/40 overflow-hidden">
                ...
                <div class="bg-kigongoniBlue px-6 py-4 flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-white font-black uppercase text-sm tracking-wide">{{ __('Standard Double Room') }}</h3>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-8 items-start">
                        ...
                        <div>
                            ...
                            <div class="flex gap-4 mt-3 px-1">
                                <div class="flex items-center gap-1.5 text-[10px] font-bold text-gray-500">
                                    <span class="w-3 h-3 rounded-sm bg-kigongoniBlue/10 border border-kigongoniBlue/40 inline-block"></span> {{ __('Low') }} $50+
                                </div>
                                <div class="flex items-center gap-1.5 text-[10px] font-bold text-gray-500">
                                    <span class="w-3 h-3 rounded-sm bg-kigongoniOrange/20 border border-kigongoniOrange/50 inline-block"></span> {{ __('High') }} $80+
                                </div>
                                <div class="flex items-center gap-1.5 text-[10px] font-bold text-gray-500">
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <div id="inline-summary" class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 space-y-3 border border-gray-100 dark:border-gray-700">
                                ...
                                <div class="flex justify-between items-center">
                                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ __('Check-Out') }}</span>
                                    <span id="inline-checkout-display" class="text-sm font-black text-kigongoniBlue dark:text-white">— {{ __('Select date') }}</span>
                                </div>
                                <div class="w-full h-px bg-gray-100 dark:bg-gray-600"></div>
                                <div class="flex justify-between items-center">
                                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ __('Nights') }}</span>
                                    <span id="inline-nights-display" class="text-sm font-black text-kigongoniOrange">—</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">{{ __('Total') }}</span>
        ...
        <div class="relative rounded-3xl overflow-hidden mb-10 shadow-2xl" data-aos="fade-up">
            ...
            <div class="relative z-10 p-8 md:p-10">
                <div class="flex flex-col lg:flex-row lg:items-stretch gap-6">
                    ...
                    {{-- LEFT: Title + Amenities (glass card) --}}
                    <div class="flex-1 rounded-2xl p-6 flex flex-col justify-between gap-6"
                        ...
                        <div>
                            ...
                            <h1 class="text-5xl md:text-6xl font-black text-white uppercase tracking-tight leading-none mb-5">
                                {{ __('Standard') }}<br>
                                <span class="text-kigongoniOrange" style="text-shadow: 0 0 40px rgba(239,74,37,0.5);">{{ __('Double Room') }}</span>
                            </h1>

                            <div class="flex flex-wrap gap-2 mb-5">
                                <span class="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border border-green-400/40 text-green-300"
                                      style="background: rgba(74,222,128,0.12); backdrop-filter: blur(8px);">
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse inline-block"></span>
                                    {{ __('Available Now') }}
                                </span>
                                <span class="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full border border-white/20 text-white/80"
                    ...
                    <div class="flex-shrink-0 lg:w-80 rounded-2xl p-6 flex flex-col gap-4"
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="flex items-start gap-0.5">
                                    ...
                                    <span id="price-display" class="text-white font-black leading-none drop-shadow-lg" style="font-size:64px;line-height:1;">50</span>
                                </div>
                                <p class="text-white/40 text-[10px] uppercase tracking-widest font-bold mt-0.5">{{ __('Per Night') }} · {{ __('Incl. B&B') }}</p>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div class="rounded-xl p-3" style="background: rgba(96,165,250,0.12); border: 1px solid rgba(96,165,250,0.3); backdrop-filter: blur(8px);">
                                <div class="flex items-center gap-1.5 mb-1.5">
                                    ...
                                    <p class="text-blue-300 text-[10px] font-black uppercase tracking-wide">{{ __('Low Season') }}</p>
                                </div>
                                <p class="text-white font-black text-xl leading-none">$50</p>
                            ...
                            <div class="rounded-xl p-3" style="background: rgba(239,74,37,0.15); border: 1px solid rgba(239,74,37,0.4); backdrop-filter: blur(8px);">
                                <div class="flex items-center gap-1.5 mb-1.5">
                                    <div class="w-2 h-2 rounded-full bg-kigongoniOrange"></div>
                                    <p class="text-kigongoniOrange text-[10px] font-black uppercase tracking-wide">{{ __('High Season') }}</p>
                                </div>
                                <p class="text-kigongoniOrange font-black text-xl leading-none">$80</p>
                                <p class="text-white/30 text-[8.5px] font-bold mt-0.5 leading-tight">Jun, Jul, Aug, Dec 15-Jan 14, Feb</p>
                            </div>
                        </div>

                        <div class="grid grid-cols-2 gap-2">
                            <div class="flex items-center gap-2 rounded-lg px-3 py-2" style="background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12);">
                                ...
                                <span class="text-white/70 text-[10px] font-bold uppercase leading-tight">{{ __('Free Cancel') }}</span>
                            </div>
                            <div class="flex items-center gap-2 rounded-lg px-3 py-2" style="background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12);">
                                ...
                                <span class="text-white/70 text-[10px] font-bold uppercase leading-tight">{{ __('Meal Upgrades') }}</span>
                            </div>
                            <div class="flex items-center gap-2 rounded-lg px-3 py-2" style="background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12);">
                                <svg class="w-4 h-4 text-blue-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <span class="text-white/70 text-[10px] font-bold uppercase leading-tight">{{ __('24/7 Support') }}</span>
                            </div>
                            <div class="flex items-center gap-2 rounded-lg px-3 py-2" style="background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12);">
                                <svg class="w-4 h-4 text-orange-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                                <span class="text-white/70 text-[10px] font-bold uppercase leading-tight">Mto wa Mbu</span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-2 mt-auto">
                            ...
                            @endif
                            <a href="https://wa.me/255768219703?text={{ urlencode(__('Hello, I would like to book the Standard Double room')) }}"
                               target="_blank"
        ...
        <div class="grid grid-cols-4 grid-rows-2 gap-3 h-[420px] md:h-[500px] mb-12 rounded-2xl overflow-hidden" data-aos="fade-up">
            ...
            <div class="col-span-4 md:col-span-2 md:row-span-2 relative overflow-hidden group cursor-pointer" onclick="openLightbox(0)">
                <img id="gallery-main"
                     src="{{ asset('images/rooms/standard-double/main.jpg') }}"
                     onerror="this.src='https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&w=1000&q=80'"
                     alt="{{ __('Standard Double - Main View') }}"
                ...
                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition duration-300 flex items-center justify-center">
                    <svg class="w-12 h-12 text-white opacity-0 group-hover:opacity-100 transition duration-300 drop-shadow-lg" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"/></svg>
                </div>
            ...
            <div class="col-span-2 md:col-span-1 relative overflow-hidden group cursor-pointer rounded-none" onclick="openLightbox(1)">
                <img src="{{ asset('images/rooms/standard-double/bathroom.jpg') }}"
                     onerror="this.src='https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?auto=format&fit=crop&w=600&q=80'"
                     alt="{{ __('Bathroom') }}"
            ...
            <div class="col-span-2 md:col-span-1 relative overflow-hidden group cursor-pointer" onclick="openLightbox(2)">
                <img src="{{ asset('images/rooms/standard-double/desk.jpg') }}"
                     onerror="this.src='https://images.unsplash.com/photo-1631049307264-da0ec9d70304?auto=format&fit=crop&w=600&q=80'"
                     alt="{{ __('Desk Area') }}"
                     class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition duration-300"></div>
            ...
            <div class="col-span-2 md:col-span-1 relative overflow-hidden group cursor-pointer" onclick="openLightbox(3)">
                <img src="{{ asset('images/rooms/standard-double/view.jpg') }}"
                     onerror="this.src='https://images.unsplash.com/photo-1540518614846-7eded433c457?auto=format&fit=crop&w=600&q=80'"
                     alt="{{ __('Room View') }}"
                     class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition duration-300"></div>
            ...
            <div class="col-span-2 md:col-span-1 relative overflow-hidden group cursor-pointer" onclick="openLightbox(4)">
                <img src="{{ asset('images/rooms/standard-double/window-side.jpg') }}"
                     onerror="this.src='https://images.unsplash.com/photo-1595599872002-9154efcf5b8e?auto=format&fit=crop&w=600&q=80'"
                     alt="{{ __('Window Side') }}"
                     class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                <div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition duration-300 flex items-center justify-center">
                    <span class="text-white text-sm font-black opacity-0 group-hover:opacity-100 uppercase tracking-widest drop-shadow-lg">{{ __('See All') }}</span>
                </div>
                <div class="absolute bottom-2 left-2 bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">{{ __('Window Side') }}</div>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div data-aos="fade-up" data-aos-delay="100">
                    <div class="flex items-center gap-3 mb-5">
                        <div class="w-8 h-1 bg-kigongoniOrange rounded-full"></div>
                        <h2 class="text-xl font-black text-kigongoniBlue dark:text-white uppercase tracking-wide">{{ __('Room Amenities') }}</h2>
                    </div>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Air Conditioning') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Free Wi-Fi') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Flat Screen TV') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Hot Shower') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Breakfast Incl.') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Mosquito Net') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Wardrobe') }}</span>
                        </div>
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                        ...
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-kigongoniOrange transition duration-300 group">
                            <div class="w-10 h-10 rounded-full bg-kigongoniOrange/10 flex items-center justify-center flex-shrink-0 group-hover:bg-kigongoniOrange/20 transition">
                                <svg class="w-5 h-5 text-kigongoniOrange" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                            </div>
                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ __('Queen/King Bed') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div data-aos="fade-up" data-aos-delay="150">
                    <div class="flex items-center gap-3 mb-5">
                        <div class="w-8 h-1 bg-kigongoniOrange rounded-full"></div>
                        <h2 class="text-xl font-black text-kigongoniBlue dark:text-white uppercase tracking-wide">{{ __('Room Details') }}</h2>
                    </div>
                    <div class="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                        <table class="w-full text-sm">
                            <tbody>
                                <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                                    <td class="px-5 py-3.5 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs w-40">{{ __('Room Size') }}</td>
                                    <td class="px-5 py-3.5 font-semibold text-gray-800 dark:text-white">~25 m²</td>
                                </tr>
                                <tr class="border-b border-gray-100 dark:border-gray-700">
                                    <td class="px-5 py-3.5 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">{{ __('Bed Type') }}</td>
                                    <td class="px-5 py-3.5 font-semibold text-gray-800 dark:text-white">{{ __('1 Double Bed (Queen/King)') }}</td>
                                </tr>
                                <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                                    <td class="px-5 py-3.5 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">{{ __('Max Guests') }}</td>
                                    <td class="px-5 py-3.5 font-semibold text-gray-800 dark:text-white">{{ __('2 Adults') }}</td>
                                </tr>
                                <tr class="border-b border-gray-100 dark:border-gray-700">
                                    <td class="px-5 py-3.5 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">{{ __('Check-In') }}</td>
                                    <td class="px-5 py-3.5 font-semibold text-gray-800 dark:text-white">{{ __('From 2:00 PM') }}</td>
                                </tr>
                                <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                                    <td class="px-5 py-3.5 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">{{ __('Check-Out') }}</td>
                                    <td class="px-5 py-3.5 font-semibold text-gray-800 dark:text-white">{{ __('By 11:00 AM') }}</td>
                                </tr>
                                <tr>
                                    <td class="px-5 py-3.5 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs">{{ __('Smoking') }}</td>
                ...
                <div data-aos="fade-up" data-aos-delay="200">
                    <div class="flex items-center gap-3 mb-5">
                        <div class="w-8 h-1 bg-kigongoniOrange rounded-full"></div>
                        <h2 class="text-xl font-black text-kigongoniBlue dark:text-white uppercase tracking-wide">{{ __('Policies') }}</h2>
                    </div>
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div class="flex gap-3 p-4 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-100 dark:border-green-800">
                            <svg class="w-5 h-5 text-green-600 dark:text-green-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                            <div>
                                <p class="text-xs font-black text-green-700 dark:text-green-400 uppercase tracking-wider mb-1">{{ __('Free Cancellation') }}</p>
                                <p class="text-xs text-gray-600 dark:text-gray-400">{{ __('Cancel up to 48 hours before arrival at no charge.') }}</p>
                            </div>
                        </div>
                        <div class="flex gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800">
                            ...
                            <div>
                                <p class="text-xs font-black text-blue-700 dark:text-blue-400 uppercase tracking-wider mb-1">{{ __('Breakfast Included') }}</p>
                                <p class="text-xs text-gray-600 dark:text-gray-400">{{ __('Complimentary breakfast served daily 7:00 AM – 10:00 AM.') }}</p>
                            </div>
                        </div>
                        <div class="flex gap-3 p-4 bg-orange-50 dark:bg-orange-900/10 rounded-xl border border-orange-100 dark:border-orange-800">
                            ...
                            <div>
                                <p class="text-xs font-black text-kigongoniOrange uppercase tracking-wider mb-1">{{ __('24/7 Front Desk') }}</p>
                                <p class="text-xs text-gray-600 dark:text-gray-400">{{ __('Our team is available around the clock for your needs.') }}</p>
                            </div>
                        </div>
                        <div class="flex gap-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700">
                            <svg class="w-5 h-5 text-gray-500 dark:text-gray-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                            <div>
                                <p class="text-xs font-black text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">{{ __('Payment') }}</p>
                                <p class="text-xs text-gray-600 dark:text-gray-400">{{ __('Cash & mobile payments accepted at check-in.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="hidden lg:block">
                <div id="booking-calendar" class="sticky top-28 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden" data-aos="fade-left">
                    <div class="bg-kigongoniBlue dark:bg-gray-900 p-5 text-white">
                        ...
                        <h3 class="text-xl font-black uppercase">{{ __('Standard Double') }}</h3>
                    </div>
                    <div class="p-4">

                        {{-- If dates came from homepage, show them + Book button --}}
                        @if($checkinFormatted && $checkoutFormatted)
                        <div class="mb-4 bg-kigongoniBlue/5 dark:bg-kigongoniBlue/20 rounded-xl p-4 space-y-2.5 border border-kigongoniBlue/15 dark:border-kigongoniBlue/30">
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Check-In') }}</span>
                                <span class="text-xs font-black text-kigongoniBlue dark:text-white">{{ $checkinFormatted }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Check-Out') }}</span>
                                <span class="text-xs font-black text-kigongoniBlue dark:text-white">{{ $checkoutFormatted }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Guests') }}</span>
                                <span class="text-xs font-black text-kigongoniBlue dark:text-white">{{ $guests }}</span>
                            </div>
                            <div class="border-t border-kigongoniBlue/10 dark:border-kigongoniBlue/30 pt-2.5 flex justify-between items-center">
                                <span class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Nights') }}</span>
                                <span class="text-sm font-black text-kigongoniOrange">{{ $nights }} {{ $nights > 1 ? __('Nights') : __('Night') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <button id="cal-prev" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition text-gray-600 dark:text-gray-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                            </button>
                            <span id="cal-title" class="text-sm font-black text-kigongoniBlue dark:text-white uppercase tracking-wide"></span>
                            <button id="cal-next" class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition text-gray-600 dark:text-gray-300">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                            </button>
                        </div>

                        <div class="grid grid-cols-7 mb-1">
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('Su') }}</div>
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('Mo') }}</div>
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('Tu') }}</div>
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('We') }}</div>
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('Th') }}</div>
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('Fr') }}</div>
                            <div class="text-center text-[9px] font-black text-gray-400 uppercase py-1">{{ __('Sa') }}</div>
                        </div>

                        <div id="cal-grid" class="grid grid-cols-7 gap-0.5"></div>

                        <div class="flex gap-3 my-3 px-1">
                            <div class="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 dark:text-gray-400">
                                <span class="w-3 h-3 rounded-sm bg-kigongoniBlue/10 border border-kigongoniBlue/40 inline-block"></span> {{ __('Low') }} $50+
                            </div>
                            <div class="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 dark:text-gray-400">
                                <span class="w-3 h-3 rounded-sm bg-kigongoniOrange/20 border border-kigongoniOrange/50 inline-block"></span> {{ __('High') }} $80+
                        ...
                        <div id="date-summary" class="hidden bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 mb-3 text-xs">
                            <div class="flex justify-between mb-1">
                                <span class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('Check-In') }}</span>
                                <span id="summary-checkin" class="font-black text-kigongoniBlue dark:text-white">—</span>
                            </div>
                            <div class="flex justify-between mb-1">
                                <span class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('Check-Out') }}</span>
                                <span id="summary-checkout" class="font-black text-kigongoniBlue dark:text-white">—</span>
                            </div>
                            <div class="border-t border-gray-200 dark:border-gray-600 mt-2 pt-2 flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('Total') }}</span>
                                <span id="summary-total" class="font-black text-kigongoniOrange text-base">—</span>
                            </div>
                        </div>
                        ...
                        <button id="cal-book-btn" class="w-full bg-kigongoniOrange text-white font-black py-3.5 rounded-xl hover:bg-kigongoniBlue dark:hover:bg-gray-900 transition duration-300 uppercase tracking-widest text-xs shadow-lg mb-3">
        ...
        <div class="block lg:hidden mb-10" data-aos="fade-up">
            <div id="booking-calendar-mobile" class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="bg-kigongoniBlue dark:bg-gray-900 p-5 text-white">
                    <p class="text-xs font-black uppercase tracking-[0.3em] text-kigongoniOrange mb-1">{{ __('Book This Room') }}</p>
                    <h3 class="text-xl font-black uppercase">{{ __('Standard Double Room') }}</h3>
                </div>

                <div class="p-4">
                    @if($checkinFormatted && $checkoutFormatted)
                    {{-- Dates from homepage: show summary + book --}}
                    <div class="mb-4 bg-kigongoniBlue/5 dark:bg-kigongoniBlue/20 rounded-xl p-4 space-y-2.5 border border-kigongoniBlue/15 dark:border-kigongoniBlue/30">
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ __('Check-In') }}</span>
                            <span class="text-sm font-black text-kigongoniBlue dark:text-white">{{ $checkinFormatted }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ __('Check-Out') }}</span>
                            <span class="text-sm font-black text-kigongoniBlue dark:text-white">{{ $checkoutFormatted }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ __('Guests') }}</span>
                            <span class="text-sm font-black text-kigongoniBlue dark:text-white">{{ $guests }}</span>
                        </div>
                        <div class="border-t border-kigongoniBlue/10 dark:border-kigongoniBlue/30 pt-2.5 flex justify-between items-center">
                            <span class="text-sm font-bold text-gray-500 dark:text-gray-400">{{ __('Nights') }}</span>
                            <span class="text-base font-black text-kigongoniOrange">{{ $nights }} {{ $nights > 1 ? __('Nights') : __('Night') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <button id="cal-prev-mob" class="w-9 h-9 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition text-gray-600 dark:text-gray-300">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                        <span id="cal-title-mob" class="text-sm font-black text-kigongoniBlue dark:text-white uppercase tracking-wide"></span>
                        <button id="cal-next-mob" class="w-9 h-9 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition text-gray-600 dark:text-gray-300">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </button>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-4 mb-4 px-1">
                        <div class="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 dark:text-gray-400">
                            <span class="w-3 h-3 rounded-sm bg-kigongoniBlue/10 border border-kigongoniBlue/40 inline-block"></span> {{ __('Low') }} $50+
                        </div>
                        <div class="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 dark:text-gray-400">
                            <span class="w-3 h-3 rounded-sm bg-kigongoniOrange/20 border border-kigongoniOrange/50 inline-block"></span> {{ __('High') }} $80+
                    ...
                    <div id="date-summary-mob" class="hidden bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 mb-4 text-sm">
                        <div class="flex justify-between mb-2">
                            <span class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('Check-In') }}</span>
                            <span id="summary-checkin-mob" class="font-black text-kigongoniBlue dark:text-white">—</span>
                        </div>
                        <div class="flex justify-between mb-2">
                            <span class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('Check-Out') }}</span>
                            <span id="summary-checkout-mob" class="font-black text-kigongoniBlue dark:text-white">—</span>
                        </div>
                        <div class="border-t border-gray-200 dark:border-gray-600 mt-2 pt-3 flex justify-between items-center">
                            <span class="text-gray-500 dark:text-gray-400 font-semibold">{{ __('Total') }}</span>
                            <span id="summary-total-mob" class="font-black text-kigongoniOrange text-lg">—</span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-3">
                        <button id="cal-book-btn-mob" class="w-full bg-kigongoniOrange text-white font-black py-4 rounded-xl hover:bg-kigongoniBlue transition duration-300 uppercase tracking-widest text-sm shadow-lg">
                            {{ __('Check Availability') }}
                        </button>
                        <a href="https://wa.me/255768219703?text={{ urlencode(__('Hello, I would like to book the Standard Double room')) }}"
                           target="_blank"
...
<div id="lightbox" class="fixed inset-0 bg-black/95 z-[100] hidden flex items-center justify-center p-4" onclick="closeLightbox(event)">
    ...
    <button onclick="prevImage()" class="absolute left-4 text-white/70 hover:text-white transition">
    ...
    <button onclick="nextImage()" class="absolute right-4 text-white/70 hover:text-white transition">
...
function isLowSeason(date) {
    const m = date.getMonth() + 1; // Miezi inaanza 1=Jan mpaka 12=Dec
    const d = date.getDate();

    // HIGH SEASON logic: June(6), July(7), Aug(8), Feb(2)
    if (m === 6 || m === 7 || m === 8 || m === 2) {
        return false;
    }
    // HIGH SEASON logic: Dec 15 to Jan 14
    if (m === 12 && d >= 15) {
        return false; // Dec 15 - Dec 31
    }
    if (m === 1 && d <= 14) {
        return false; // Jan 1 - Jan 14
    }

    // LOW SEASON logic: Tarehe na miezi iliyobaki yote ni Low Season
    return true; 
}
...
// Update hero price based on today's season
(function() {
...
// Calculate total with mixed days and meal plans
...
// Update summaries for users who came with pre-filled dates
function updatePreFilledSummaries() {
    ...
    if (checkinStr !== "" && checkoutStr !== "") {
        ...
        // Ensure dropdowns have right options
...
function buildGrid(gridEl, isMobile) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const price      = getDayPrice(date, 'BB'); // Show base price on cal
        ...
        let base = isMobile
            ? 'relative flex flex-col items-center justify-center rounded-lg py-1.5 px-0.5 text-center transition duration-150 w-full '
            : 'relative flex flex-col items-center justify-center rounded-lg py-1 px-0.5 text-center transition duration-150 ';

        if (isPast) {
            base += 'opacity-30 cursor-not-allowed ';
        } else if (isCheckin || isCheckout) {
            base += 'bg-kigongoniOrange text-white shadow-md scale-105 z-10 ';
        } else if (isInRange) {
            base += 'bg-kigongoniOrange/30 text-kigongoniOrange dark:text-orange-300 border border-kigongoniOrange/50 ';
        } else if (low) {
            base += 'bg-kigongoniBlue/5 dark:bg-kigongoniBlue/20 hover:bg-kigongoniOrange/20 hover:border-kigongoniOrange text-gray-700 dark:text-gray-200 border border-kigongoniBlue/20 ';
        } else {
            base += 'bg-kigongoniOrange/5 dark:bg-kigongoniOrange/10 hover:bg-kigongoniOrange/20 hover:border-kigongoniOrange text-gray-700 dark:text-gray-200 border border-kigongoniOrange/20 ';
        }

        cell.className = base;

        const priceColor = isCheckin||isCheckout
            ? 'text-white/90'
            : isInRange
                ? 'text-kigongoniOrange font-black'
                : (low ? 'text-kigongoniBlue dark:text-blue-300' : 'text-kigongoniOrange');

        const numSize  = isMobile ? 'text-[12px]' : 'text-[11px]';
        const priceSize = isMobile ? 'text-[9px]'  : 'text-[8px]';

        cell.innerHTML = `
            <span class="${numSize} font-black leading-none">${d}</span>
            <span class="${priceSize} font-bold leading-none mt-0.5 ${priceColor}">$${price}</span>
        `;
...
function renderCalendar() {
    ...
    const titleD = document.getElementById('cal-title');
    if (titleD) { titleD.textContent = label; buildGrid(document.getElementById('cal-grid'), false); }

    const titleM = document.getElementById('cal-title-mob');
    if (titleM) { titleM.textContent = label; buildGrid(document.getElementById('cal-grid-mob'), true); }
}
...
function onDayClick(date) {
    ...
    if (!checkIn || (checkIn && checkOut)) {
        ...
        if (date <= checkIn) {
            ...
            // Populate days dropdown once checkout is selected
...
function updateSummary() {
    ...
    const nights = Math.round((checkOut - checkIn) / 86400000);
    const nightText = nights > 1 ? "{{ __('Nights') }}" : "{{ __('Night') }}";
    ...
    if (sumD) {
        document.getElementById('summary-checkin').textContent  = fmt(checkIn);
        document.getElementById('summary-checkout').textContent = fmt(checkOut);
        document.getElementById('summary-total').textContent    = totalStr + ' (' + nights + ' ' + nightText + ')';
        sumD.classList.remove('hidden');
    }

    const sumM = document.getElementById('date-summary-mob');
    if (sumM) {
        document.getElementById('summary-checkin-mob').textContent  = fmt(checkIn);
        document.getElementById('summary-checkout-mob').textContent = fmt(checkOut);
        document.getElementById('summary-total-mob').textContent    = totalStr;
...
function handleBookClick(isPrefilled = false) {
    ...
    if(isPrefilled) {
        ...
        finalGuests = document.getElementById('cal-guests')?.value || document.getElementById('cal-guests-mob')?.value || '2 Adults';
    }
...
// Attach to fresh select buttons
...
document.getElementById('cal-prev-mob')?.addEventListener('click', () => {
    calMonth--; if (calMonth < 0) { calMonth = 11; calYear--; } renderCalendar();
});
document.getElementById('cal-next-mob')?.addEventListener('click', () => {
    calMonth++; if (calMonth > 11) { calMonth = 0; calYear++; } renderCalendar();
});
...
let inlineYear  = new Date().getFullYear();
let inlineMonth = new Date().getMonth();
let inlineCheckIn  = null;
let inlineCheckOut = null;
let inlineSelectingOut = false;
...
function toggleInlineCal() {
    ...
    if (isHidden) {
        ...
        inlineYear  = new Date().getFullYear();
        inlineMonth = new Date().getMonth();
        renderInlineCal();
        setTimeout(() => {
...
function buildInlineGrid() {
    const grid = document.getElementById('inline-cal-grid');
    if (!grid) return;
    grid.innerHTML = '';
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        const date = new Date(inlineYear, inlineMonth, d);
        date.setHours(0, 0, 0, 0);
        ...
        if (isDisabled) {
            ...
            cls += 'bg-kigongoniOrange text-white shadow-md scale-105 z-10 ';
        } else if (isInRange) {
            cls += 'bg-kigongoniOrange/25 text-kigongoniOrange border border-kigongoniOrange/40 ';
        } else if (low) {
            cls += 'bg-kigongoniBlue/5 dark:bg-kigongoniBlue/20 hover:bg-kigongoniOrange/20 text-gray-700 dark:text-gray-200 border border-kigongoniBlue/20 hover:border-kigongoniOrange ';
        } else {
            cls += 'bg-kigongoniOrange/5 dark:bg-kigongoniOrange/10 hover:bg-kigongoniOrange/20 text-gray-700 dark:text-gray-200 border border-kigongoniOrange/20 hover:border-kigongoniOrange ';
        }
        cell.className = cls;

        const priceColor = (isCI||isCO) ? 'text-white/90' : (low ? 'text-kigongoniBlue dark:text-blue-300' : 'text-kigongoniOrange');
        cell.innerHTML = `
            <span class="text-[11px] font-black leading-none">${d}</span>
            <span class="text-[8px] font-bold leading-none mt-0.5 ${priceColor}">$${price}</span>
        `;
        if (!isDisabled) cell.addEventListener('click', () => onInlineDayClick(date));
...
function updateInlineSummary() {
    ...
    if (inlineCheckIn && inlineCheckOut) {
        ...
        if (niEl) niEl.textContent = nights + ' ' + nightText;
...
document.getElementById('inline-confirm-btn')?.addEventListener('click', () => {
    ...
    const params = new URLSearchParams({
        checkin:    toISO(inlineCheckIn),
        checkout:   toISO(inlineCheckOut),
        guests:     guests,
        room_type:  'Standard Double Room',
...
document.getElementById('inline-cal-next')?.addEventListener('click', () => {
    inlineMonth++;
    if (inlineMonth > 11) { inlineMonth = 0; inlineYear++; }
    renderInlineCal();
});

// ─── Lightbox ─────────────────────────────────────────────────
const galleryImages = [
    { src: "{{ asset('images/rooms/standard-double/main.jpg') }}",        fallback: 'https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&w=1200&q=80', label: "{{ __('Main View') }}" },
    { src: "{{ asset('images/rooms/standard-double/bathroom.jpg') }}",    fallback: 'https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?auto=format&fit=crop&w=1200&q=80', label: "{{ __('Bathroom') }}" },
    { src: "{{ asset('images/rooms/standard-double/desk.jpg') }}",        fallback: 'https://images.unsplash.com/photo-1631049307264-da0ec9d70304?auto=format&fit=crop&w=1200&q=80', label: "{{ __('Desk Area') }}" },
    { src: "{{ asset('images/rooms/standard-double/view.jpg') }}",        fallback: 'https://images.unsplash.com/photo-1540518614846-7eded433c457?auto=format&fit=crop&w=1200&q=80', label: "{{ __('Room View') }}" },
    { src: "{{ asset('images/rooms/standard-double/window-side.jpg') }}", fallback: 'https://images.unsplash.com/photo-1595599872002-9154efcf5b8e?auto=format&fit=crop&w=1200&q=80', label: "{{ __('Window Side') }}" },
];
let currentImg = 0;
...
function showLightboxImage() {
    ...
    const item = galleryImages[currentImg];
    img.src = item.src;
    img.onerror = () => { img.src = item.fallback; img.onerror = null; };
    document.getElementById('lightbox-counter').textContent =
        item.label + '  ·  ' + (currentImg + 1) + ' / ' + galleryImages.length;
}
